...
Symptom-based triage and differential support.

## Standalone Migration Status

This section is being rebuilt from the Notion Symptom Sorter so that the public wiki can be used without opening Notion.
...
The remaining symptom pages still need body-content import, local anchors, and cross-link conversion before this batch can be marked complete.
...
- Each symptom gets its own local Markdown page.
- The page must contain triage content, differentials, red flags, and medication/practice notes when present.
- Related conditions are written as local text first. Links are added only when a local target exists.
- Notion page links are not used as the reader-facing navigation model.
